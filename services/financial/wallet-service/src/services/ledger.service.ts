...
export const ledgerService = {
   ...
   */
  async creditWallet(request: LedgerWriteRequest, tx?: Prisma.TransactionClient): Promise<LedgerWriteResponse> {
    // Validate amount
   ...
   */
  async debitWallet(request: LedgerWriteRequest, tx?: Prisma.TransactionClient): Promise<LedgerWriteResponse> {
    // Validate amount
    if (!isPositiveAmount(request.amount)) {
      throw new InvalidAmountError('Amount must be greater than 0');
    }
  ...
  async executeAtomicWrite(
    ...
    const work = async (tx: Prisma.TransactionClient) => {
      ...
      if (wallet.status === 'FROZEN') {
        throw new WalletFrozenError(request.walletId);
      }
      if (wallet.status === 'CLOSED') {
        throw new WalletClosedError(request.walletId);
      }
      ...
      if (systemControl.length > 0 && systemControl[0].value === 'PAUSED') {
        throw new Error('SYSTEM_FINANCIAL_MODE_PAUSED: All money movement is currently halted.');
      }
      ...
      // --------------------------------------------------------
      const lastEntryRows = await tx.$queryRaw<Array<{
      ...
      const currentBalance = lastEntryRows.length > 0 
        ? BigInt(lastEntryRows[0].balance_after)
        : BigInt(0);
      ...
      if (request.entryType === EntryType.CREDIT) {
        ...
        if (currentBalance < request.amount) {
          throw new InsufficientBalanceError(currentBalance, request.amount);
        }
        newBalance = currentBalance - request.amount;
      }
  ...
  /**
   * Get balance before this entry was created
   */
  async getBalanceBefore(entry: LedgerEntry): Promise<bigint> {
    if (entry.entryType === EntryType.CREDIT) {
      return entry.balanceAfter - entry.amount;
    } else {
      return entry.balanceAfter + entry.amount;
    }
  },
  ...
  toWriteResponse(entry: LedgerEntry, isIdempotent: boolean): LedgerWriteResponse {
    const balanceBefore = entry.entryType === EntryType.CREDIT
