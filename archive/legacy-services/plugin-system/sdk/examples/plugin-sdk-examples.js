...
exports.basicPluginExample = basicPluginExample;
exports.fluentPluginExample = fluentPluginExample;
exports.templatePluginExample = templatePluginExample;
exports.devUtilsExample = devUtilsExample;
...
async function basicPluginExample() {
    ...
    const paymentPlugin = new src_1.PluginSDK({
...
async function fluentPluginExample() {
    ...
    const analyticsPlugin = await new src_1.PluginBuilder({
        ...
        const eventData = await Promise.all(events.slice(0, 10).map(key => context.storage.get(key)));
        return {
            success: true,
...
async function templatePluginExample() {
    ...
    const templates = src_1.PluginDevUtils.getPluginTemplates();
    ...
    const emailPlugin = await src_1.PluginBuilder.fromTemplate(templates.find(t => t.id === 'email-marketing'), {
...
async function devUtilsExample() {
    ...
    // Validate manifest
    const validation = src_1.PluginDevUtils.validateManifest(sampleManifest);
    console.log('Manifest validation:', validation);
    // Generate plugin template code
    const templateCode = src_1.PluginDevUtils.generatePluginTemplate('payment', {
...
//# sourceMappingURL=plugin-sdk-examples.js.map