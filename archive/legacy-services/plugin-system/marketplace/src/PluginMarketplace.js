...
exports.PluginMarketplace = void 0;
class PluginMarketplace {
    ...
    async searchPlugins(options = {}) {
        const { query, category, tags, minRating, verifiedOnly, sortBy = 'name', sortOrder = 'asc', limit = 20, offset = 0, } = options;
        const where = {};
        // Text search
        if (query) {
            where.OR = [
     ...
     */
    async getRatingDistribution(pluginId) {
     ...
     * Update plugin average rating
     */
    async updatePluginAverageRating(pluginId) {
     ...
     */
    async recordDownload(pluginId, userId, version) {
    ...
    async getRecommendations(userId, limit = 10) {
        ...
        const downloadedPluginIds = userDownloads.map((d) => d.pluginId);
    ...
    async getMarketplaceStats() {
        const [totalPlugins, totalDownloads, totalPublishers, averageRating, topCategories, recentPlugins, popularPlugins,] = await Promise.all([
     ...
     */
    async createOrUpdatePublisher(data) {
     ...
     */
    async checkCompatibility(pluginId, coreVersion) {
        const plugin = await this.prisma.pluginMarketplace.findUnique({
            where: { id: pluginId },
            include: {
                versions: {
