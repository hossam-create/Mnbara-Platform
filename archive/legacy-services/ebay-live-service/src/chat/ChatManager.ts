...
export class ChatManager extends EventEmitter {
  ...
  private moderationQueue: Map<string, any[]> = new Map();
   ...
   * Get chat messages
   */
  async getMessages(filters: ChatFilters): Promise<any[]> {
    try {
      const where: any = { streamId: filters.streamId };
   ...
   * Delete a message
   */
  async deleteMessage(messageId: string, userId: string, isModerator: boolean = false): Promise<void> {
    try {
   ...
   * Mute a user
   */
  async muteUser(streamId: string, userId: string, duration: number = 300): Promise<void> {
    try {
   ...
   * Check if user is muted
   */
  private async isUserMuted(streamId: string, userId: string): Promise<boolean> {
    try {
      const muteKey = `mute:${streamId}:${userId}`;
   ...
   * Moderate message
   */
  private async moderateMessage(message: any): Promise<void> {
    try {
   ...
   */
  private async getBannedWords(streamId: string): Promise<string[]> {
    try {
   ...
   */
  private async hasModerationPermission(streamId: string, userId: string): Promise<boolean> {
    try {
   ...
   */
  private async updateRoomStats(streamId: string): Promise<void> {
    try {
   ...
   */
  async getRoomStats(streamId: string): Promise<any> {
    try {
      const statsKey = `chat:stats:${streamId}`;
   ...
   * Join chat room
   */
  async joinRoom(streamId: string, userId: string): Promise<void> {
    try {
      const room = this.getOrCreateRoom(streamId);
   ...
   */
  async setRoomModeration(streamId: string, enabled: boolean): Promise<void> {
    try {
      const room = this.getOrCreateRoom(streamId);
