...
export class RealtimeBidService {
  ...
  /**
   * Buy Now: instant purchase at buyNowPrice with concurrency safety.
   * Integrated from Real-Time-Bike-Auction-System-Backend auctionService.buyNow().
   */
  async buyNow(listingId: number, userId: number) {
    return await this.prisma.$transaction(async (tx) => {
      const listing = await tx.listing.findUnique({
        where: { id: listingId },
        include: {
      ...
      if (!listing.buyNowPrice || listing.buyNowPrice.lte(0)) {
        throw new BadRequestError('This auction does not have a Buy Now price');
      }
      if (listing.sellerId === userId) {
        throw new BadRequestError('Seller cannot buy their own item');
      }
      ...
      await tx.listing.update({
        where: { id: listingId },
        data: {
          status: 'ENDED_AWAITING_SETTLEMENT',
          winnerId: userId,
      ...
      return { bid, auction: await tx.listing.findUnique({ where: { id: listingId } }) };
    });
  }
}
