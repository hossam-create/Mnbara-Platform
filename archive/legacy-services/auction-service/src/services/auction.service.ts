...
export class AuctionService {
  ...
  /**
   * Start scheduled auctions whose start time has passed (for cron job).
   * Integrated from Real-Time-Bike-Auction-System-Backend: SCHEDULED → LIVE transition.
   */
  async startScheduledAuctions(): Promise<{ started: number; listingIds: number[] }> {
    const now = new Date();
    const toStart = await prisma.listing.findMany({
      where: {
        isAuction: true,
    ...
    for (const { id } of toStart) {
      try {
        await this.startAuction(id);
        listingIds.push(id);
      } catch (e) {
        // log and continue
      }
    }
    return { started: listingIds.length, listingIds };
  }
