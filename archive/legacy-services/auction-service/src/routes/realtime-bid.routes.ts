...
/**
 * Buy Now: instant purchase at buyNowPrice (integrated from Real-Time-Bike-Auction).
 * POST /auctions/:auctionId/buy-now
 */
router.post('/:auctionId/buy-now', async (req: Request, res: Response) => {
  try {
    const auctionId = parseInt(req.params.auctionId);
    const userId = (req as any).user?.id || 1;

    const { bid, auction } = await realtimeBidService.buyNow(auctionId, userId);
    ...
    res.status(201).json({
      success: true,
      data: {
        bid: { id: bid.id, amount: bid.amount, createdAt: bid.createdAt },
        auction: auction ? { id: auction.id, status: auction.status, finalPrice: auction.finalPrice, winnerId: auction.winnerId } : null,
      },
    });
  } catch (error: any) {
    res.status(error.name === 'BadRequestError' ? 400 : 500).json({
      success: false,
      error: error.message || 'Buy Now failed',
    });
  }
});
