...
const auctionCheckJob = setInterval(async () => {
  try {
    // SCHEDULED → ACTIVE (integrated from Real-Time-Bike-Auction)
    const startResult = await auctionService.startScheduledAuctions();
    if (startResult.started > 0) {
      console.log(`[AUCTION_CHECK] Started ${startResult.started} scheduled auctions: ${startResult.listingIds.join(', ')}`);
    }
