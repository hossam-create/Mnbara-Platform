# Plain-Language Legal Copy Guide

Legal text rewritten to be friendly, non-threatening, and understandable by everyone.

---

## Writing Principles

| Instead of... | Use... |
|---------------|--------|
| "You agree to be bound by" | "Here's what you're agreeing to" |
| "Pursuant to" | "Based on" or just remove it |
| "Hereby" | Just remove it |
| "Notwithstanding" | "Even if" or "Regardless of" |
| "Shall" | "Will" |
| "Terminate" | "End" or "Cancel" |
| "Indemnify" | "Cover the costs if something goes wrong" |
| "Liability" | "Responsibility" |
| "Breach" | "Break" or "Violate" |
| "Pursuant to applicable law" | "As required by law" |
| "In the event that" | "If" |
| "Prior to" | "Before" |
| "Subsequent to" | "After" |
| "Utilize" | "Use" |
| "Commence" | "Start" |

---

## 1. Terms of Service

### Opening

**Before (Legal):**
> By accessing or using our platform, you agree to be bound by these Terms of Service and all applicable laws and regulations. If you do not agree with any of these terms, you are prohibited from using or accessing this site.

**After (Plain Language):**
> Welcome to mnbarh. These are the rules for using our platform. We've tried to make them clear and fair. If something doesn't make sense, ask us.

---

### Account Registration

**Before (Legal):**
> To use certain features of our platform, you must register for an account. You agree to provide accurate and complete information, maintain the security of your account credentials, notify us immediately of any unauthorized access, and be responsible for all activities under your account.

**After (Plain Language):**

**Your Account**

When you create an account, we need a few things from you:

- **Be honest** â€” Use your real information. We need this to keep everyone safe.
- **Keep your password safe** â€” Don't share it. If someone else uses your account, you're responsible for what happens.
- **Tell us if something's wrong** â€” If you think someone got into your account, let us know right away. We'll help you fix it.

---

### Marketplace Rules

**Before (Legal):**
> As a user of our marketplace, you agree to list only items you have the right to sell, provide accurate descriptions of items, honor all completed transactions, not engage in fraudulent or deceptive practices, and comply with all applicable laws and regulations.

**After (Plain Language):**

**Selling on mnbarh**

If you're selling something:

- **Only sell what's yours** â€” Don't list items you don't own or can't legally sell.
- **Be accurate** â€” Describe items honestly. If there's a scratch, say so. Photos should show the real item.
- **Follow through** â€” If someone buys from you, ship it. Backing out hurts everyone.
- **Play fair** â€” No fake listings, no misleading descriptions, no tricks.

If you're buying:

- **Pay for what you buy** â€” When you commit to a purchase, follow through.
- **Communicate** â€” If there's an issue, talk to the seller first. Most problems can be solved with a conversation.

---

### Auction Rules

**Before (Legal):**
> When participating in auctions, all bids are binding commitments to purchase. Bid manipulation or shill bidding is prohibited. Winners must complete payment within 48 hours. Sellers must ship within the stated timeframe.

**After (Plain Language):**

**How Auctions Work**

- **Bids are real commitments** â€” When you bid, you're saying "I'll buy this at this price." Don't bid if you don't mean it.
- **No fake bidding** â€” Don't bid on your own items or have friends bid to drive up prices. We can tell, and we'll remove you from the platform.
- **Winners pay within 48 hours** â€” If you win, complete your payment within 2 days.
- **Sellers ship on time** â€” When your item sells, ship it within the timeframe you promised.

---

### Crowdshipping

**Before (Legal):**
> For travelers participating in crowdshipping, complete KYC verification is required. You must comply with all customs regulations. Delivery evidence must be provided. Escrow funds are released upon confirmed delivery.

**After (Plain Language):**

**Crowdshipping (Travelers Helping Shoppers)**

If you're a traveler delivering items:

- **Verify your identity first** â€” We need to confirm who you are before you can deliver. This protects everyone.
- **Know the customs rules** â€” Different countries have different rules about what you can bring in. It's your responsibility to know them.
- **Take photos** â€” When you deliver, take a photo as proof. This protects you if there's ever a question.
- **Payment comes after delivery** â€” We hold the money safely until the buyer confirms they got their item. Then you get paid.

---

### Payments and Fees

**Before (Legal):**
> mnbarh charges fees for certain services. All fees are disclosed before transactions. Payment processing is handled securely through our approved payment providers.

**After (Plain Language):**

**What It Costs**

- **We show you fees upfront** â€” Before you complete any transaction, you'll see exactly what you're paying. No surprises.
- **Your payment is secure** â€” We use trusted payment providers to keep your money safe.
- **We take a small cut** â€” That's how we keep the platform running. The exact amount depends on the transaction type, and we always show it before you pay.

---

### Dispute Resolution

**Before (Legal):**
> In case of disputes between buyers and sellers, mnbarh provides a resolution process. Our decisions in dispute cases are final and binding.

**After (Plain Language):**

**If Something Goes Wrong**

We hope every transaction goes smoothly, but sometimes things don't work out.

- **Talk first** â€” Try to work it out with the other person. Most issues are misunderstandings.
- **We're here to help** â€” If you can't resolve it, open a dispute. We'll look at both sides and make a fair decision.
- **Our decision is final** â€” We do our best to be fair, but we can't keep going back and forth forever. Once we decide, that's the answer.

---

### Limitation of Liability

**Before (Legal):**
> mnbarh is not liable for any indirect, incidental, or consequential damages arising from your use of the platform. Our total liability is limited to the fees you have paid to us.

**After (Plain Language):**

**What We're Responsible For**

Let's be honest about what we can and can't do:

- **We're a marketplace, not a guarantee** â€” We connect buyers and sellers, but we can't control what every person does.
- **We're not responsible for everything** â€” If something goes wrong that we couldn't have prevented, we can't cover all the costs.
- **We'll make it right when we can** â€” If we mess up, we'll refund the fees you paid us. That's the most we can offer.

We're not trying to avoid responsibilityâ€”we just want to be clear about what's realistic.

---

## 2. Privacy Policy

### Opening

**Before (Legal):**
> At mnbarh, we are committed to protecting your privacy and ensuring the security of your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our marketplace platform.

**After (Plain Language):**

**Your Privacy Matters**

We take your privacy seriously. This page explains what information we collect, why we need it, and what we do with it. No legal jargon, just straight answers.

---

### Information We Collect

**Before (Legal):**
> We collect information you provide directly to us, such as when you create an account, make a purchase, list an item for sale, or contact us for support. This may include name and contact information, payment and billing information, identity verification documents, location data for delivery services, and communication preferences.

**After (Plain Language):**

**What We Collect**

Here's what we know about you and why:

| What | Why We Need It |
|------|----------------|
| **Your name and email** | So we can contact you and you can log in |
| **Your phone number** | For delivery updates and account security |
| **Payment info** | To process your purchases (we don't store full card numbers) |
| **Your ID** (for travelers) | To verify you are who you say you are |
| **Your location** (when delivering) | So buyers know where their package is |
| **What you browse** | To show you relevant items (you can turn this off) |

---

### How We Use Your Information

**Before (Legal):**
> We use the information we collect to process transactions and send related information, verify your identity and prevent fraud, provide customer support, send promotional communications (with your consent), and improve our services and develop new features.

**After (Plain Language):**

**What We Do With It**

- **Run the platform** â€” Process your orders, send receipts, handle deliveries
- **Keep things safe** â€” Verify identities, catch fraud, protect your account
- **Help you** â€” Answer your questions, resolve problems
- **Make things better** â€” Understand what's working and what's not
- **Send updates** â€” Only if you want them (you can opt out anytime)

**What we DON'T do:**
- Sell your personal information
- Share your data with advertisers
- Keep data longer than we need to

---

### Information Sharing

**Before (Legal):**
> We may share your information with third parties only in the following circumstances: with your consent, to complete transactions you have requested, with service providers who assist our operations, and to comply with legal obligations.

**After (Plain Language):**

**Who Sees Your Information**

| Who | What They See | Why |
|-----|---------------|-----|
| **Buyers/Sellers you transact with** | Name, delivery address (for shipping) | To complete the transaction |
| **Payment processors** | Payment details | To process your payment securely |
| **Delivery partners** | Delivery address, phone | To get your package to you |
| **Our support team** | Account details | To help you when you need it |
| **Law enforcement** | Whatever they legally require | Only when we're legally required to |

We never sell your data. Period.

---

### Data Security

**Before (Legal):**
> We implement appropriate technical and organizational measures to protect your personal information against unauthorized access, alteration, disclosure, or destruction.

**After (Plain Language):**

**How We Protect Your Data**

- **Encryption** â€” Your data is scrambled so only we can read it
- **Secure servers** â€” We use industry-standard protection
- **Limited access** â€” Only team members who need your data can see it
- **Regular checks** â€” We test our security regularly

No system is 100% secure, but we work hard to protect you.

---

### Your Rights

**Before (Legal):**
> You have the right to access, correct, or delete your personal information. You may also object to or restrict certain processing of your data. Contact us to exercise these rights.

**After (Plain Language):**

**Your Data, Your Choice**

You can:

- **See your data** â€” Request a copy of everything we have about you
- **Fix mistakes** â€” Update or correct your information anytime
- **Delete your account** â€” Ask us to remove your data (some things we have to keep for legal reasons)
- **Opt out** â€” Turn off marketing emails, personalization, or analytics
- **Ask questions** â€” Email us at privacy@mnbarh.com

We'll respond within 30 days.

---

## 3. Data Consent

### Essential Data

**Before (Legal):**
> Required for core platform functionality including account management, transactions, and security.

**After (Plain Language):**

**Essential Data** (Required)

This is the minimum we need to run the platform:
- Your account info
- Transaction records
- Security logs

Without this, the app can't work. That's why it's required.

---

### Analytics

**Before (Legal):**
> Help us improve the platform by sharing anonymous usage data and crash reports.

**After (Plain Language):**

**Analytics** (Optional)

This helps us understand:
- Which features people use
- Where the app crashes
- What's confusing

It's anonymousâ€”we see patterns, not individuals. Turn it off if you prefer.

---

### Personalization

**Before (Legal):**
> Allow us to personalize your feed, recommendations, and search results based on your activity.

**After (Plain Language):**

**Personalization** (Optional)

This lets us:
- Show you items you might like
- Remember your preferences
- Make search results more relevant

If you turn this off, you'll see the same thing as everyone else.

---

### Marketing

**Before (Legal):**
> Receive promotional emails, special offers, and updates about new features.

**After (Plain Language):**

**Marketing Emails** (Optional)

We'll send you:
- Special deals
- New feature announcements
- Tips for using the platform

Usually 1-2 emails per week, max. Unsubscribe anytime with one click.

---

## 4. Escrow Explanation

**Before (Legal):**
> Funds are held in escrow pending successful completion of the transaction and confirmation of delivery by the buyer. Release of funds is subject to the terms and conditions of the escrow agreement.

**After (Plain Language):**

**How Payment Protection Works**

1. **Buyer pays** â€” Money goes into a secure holding account (not to the seller yet)
2. **Seller/Traveler delivers** â€” Item gets shipped and delivered
3. **Buyer confirms** â€” Buyer says "yes, I got it"
4. **Seller gets paid** â€” Money is released to the seller

If something goes wrong, the money is still safe while we figure it out.

---

## 5. Refund Policy

**Before (Legal):**
> Refunds are subject to the terms of the original transaction and may be issued at the sole discretion of mnbarh following investigation of the dispute. Processing times may vary.

**After (Plain Language):**

**Refunds**

- **Item never arrived?** â€” Full refund
- **Item not as described?** â€” Full or partial refund depending on the situation
- **Changed your mind?** â€” Depends on the seller's policy (check before buying)
- **How long?** â€” Usually 3-5 business days back to your original payment method

If you're not sure, just ask. We'll explain your options.

---

## 6. Account Termination

**Before (Legal):**
> mnbarh reserves the right to terminate or suspend your account at any time, with or without cause, with or without notice. Upon termination, your right to use the platform will immediately cease.

**After (Plain Language):**

**Account Suspension or Closure**

We might close your account if you:
- Break the rules repeatedly
- Commit fraud
- Harass other users
- Do something illegal

We'll usually warn you first, unless it's serious. If we close your account:
- You can't use the platform anymore
- Pending transactions will be handled fairly
- You can appeal if you think we made a mistake

---

## 7. Changes to Terms

**Before (Legal):**
> We reserve the right to modify these terms at any time. Your continued use of the platform following the posting of changes constitutes your acceptance of such changes.

**After (Plain Language):**

**If We Change These Rules**

Sometimes we need to update these terms. When we do:
- We'll email you about important changes
- We'll give you time to review before they take effect
- If you don't agree, you can close your account

We won't make changes that hurt you without good reason.

---

## Quick Reference: Tone Transformation

| Legal Tone | Plain Language |
|------------|----------------|
| "Failure to comply will result in termination" | "If you break these rules, we might close your account" |
| "You hereby grant us a license" | "You're giving us permission to" |
| "We disclaim all warranties" | "We can't promise everything will be perfect" |
| "You shall indemnify us" | "If we get in trouble because of something you did, you'll help cover the costs" |
| "Subject to applicable law" | "As the law requires" |
| "At our sole discretion" | "We'll decide" |
| "Without limitation" | (Just remove it) |
| "Including but not limited to" | "Like" or "Such as" |

---

## Implementation Notes

### For Developers

When displaying legal text in the app:
- Use the plain-language versions from this document
- Keep a link to the full legal version for those who want it
- Use expandable sections for longer explanations
- Add tooltips for any remaining technical terms

### For Legal Review

These plain-language versions should be reviewed by legal counsel to ensure they:
- Maintain legal enforceability
- Cover all required disclosures
- Comply with regional regulations (GDPR, CCPA, etc.)

The goal is clarity without losing protection.

---

*Document Version: 1.0*
*Last Updated: December 2025*
*Owner: Legal & Product Teams*


