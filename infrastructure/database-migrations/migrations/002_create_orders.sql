...
-- Description: Creates the orders table and related order management tables
-- Created: 2024-01-01
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    status VARCHAR(30) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'confirmed', 'processing', 'ready', 'shipped', 'delivered', 'completed', 'cancelled', 'refunded', 'disputed')),
    ...
    payment_status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (payment_status IN ('pending', 'processing', 'paid', 'partially_refunded', 'refunded', 'failed', 'cancelled')),
...
CREATE TABLE IF NOT EXISTS order_items (
    ...
    vendor_id UUID REFERENCES users(id),
    vendor_name VARCHAR(200),
    status VARCHAR(30) DEFAULT 'pending' CHECK (status IN ('pending', 'confirmed', 'processing', 'ready', 'shipped', 'delivered', 'returned', 'cancelled')),
...
CREATE TABLE IF NOT EXISTS order_returns (
    ...
    reason VARCHAR(100) NOT NULL,
    reason_detail TEXT,
    status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected', 'label_created', 'in_transit', 'received', 'inspected', 'refunded', 'completed', 'cancelled')),
