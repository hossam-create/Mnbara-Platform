...
export default function AuctionCard({ 
  ...
  const [countdown, setCountdown] = useState(() =>
    formatCountdown(auctionService.getAuctionTimeRemaining({ auctionEndsAt: auction.auctionEndsAt }))
  );

  useEffect(() => {
    const timer = setInterval(() => {
      const newCountdown = formatCountdown(
        auctionService.getAuctionTimeRemaining({ auctionEndsAt: auction.auctionEndsAt })
      );
    ...
    >
      <Link to={`/auctions/${auction.id}`} className={styles.cardLink}>
        ...
        <div className={styles.contentSection}>
          ...
          {(auction.status === AuctionStatus.ENDED || auction.status === AuctionStatus.SOLD) && (
