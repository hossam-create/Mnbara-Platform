...
if %errorlevel% neq 0 (
    echo â‌Œ PostgreSQL is not running. Please start PostgreSQL first.
...
psql -h localhost -p 5432 -U postgres -d mnbarh_platform -f scripts/setup-owner-accounts.sql
if %errorlevel% neq 0 (
    echo â‌Œ Failed to create owner accounts
    pause
    exit /b 1
)
...
echo.
echo ============================================
echo ًںژ‰ Setup Complete!
echo ============================================
echo.
echo ًں“ٹ Admin Dashboard (Business Management):
echo URL: http://localhost:3000
echo Email: owner@mnbarh.com
echo Password: mnbarhOwner2026!
echo Role: SUPER_ADMIN (Full Access)
echo.
echo ًںڑ€ System Control Dashboard (Technical):
...
echo.
echo ًں”گ MFA Backup Codes (save these):
echo 123456, 789012, 345678, 901234, 567890
echo.
echo ًں“± MFA Setup Instructions:
