...
const elasticsearch_1 = require("../../backend/services/shared/elasticsearch");
const prisma = new client_1.PrismaClient();
const indexingService = new elasticsearch_1.IndexingService();
...
async function bulkIndexListings() {
    console.log('\n📋 Indexing Listings...');
    let total = 0;
    let indexed = 0;
    let failed = 0;
...
async function bulkIndexAuctions() {
    console.log('\n🔨 Indexing Auctions...');
    let total = 0;
    let indexed = 0;
    let failed = 0;
    ...
    while (true) {
        ...
        const documents = auctions.map((auction) => ({
            ...
            images: auction.listing.product.images,
...
async function bulkIndexCategories() {
    console.log('\n📁 Indexing Categories...');
...
async function main(options = {}) {
    ...
    try {
        ...
        if (options.recreate) {
            console.log('\n🔄 Recreating indices...');
            const indicesToRecreate = options.indices || Object.values(elasticsearch_1.INDICES);
            for (const index of indicesToRecreate) {
                await (0, elasticsearch_1.recreateIndex)(index);
            }
        }
        else {
        ...
        console.log('\nIndex Statistics:');
        console.log('─'.repeat(50));
        const stats = await (0, elasticsearch_1.getAllIndicesStatus)();
...
main(options);
//# sourceMappingURL=bulk-index.js.map