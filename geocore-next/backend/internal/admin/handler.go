package admin
  ...
  func (h *Handler) BanUser(c *gin.Context) {
        ...
        result := h.db.Model(&users.User{}).Where("id = ?", c.Param("id")).
                Updates(map[string]any{"is_banned": true, "ban_reason": req.Reason})
        if result.RowsAffected == 0 {
                response.NotFound(c, "user")
                return
        }
        h.logAction(c, "ban_user", "user", c.Param("id"), map[string]string{"reason": req.Reason})
  ...
  // suppress unused import warning — strings is used in BanUser reason
