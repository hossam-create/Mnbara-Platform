package auth
  ...
  func (h *Handler) VerifyEmail(c *gin.Context) {
        ...
        // Issue a fresh token pair so the client gets an updated session
        accessToken, _ := generateAccessToken(user.ID.String(), user.Email, user.Role)
        refreshToken, _ := h.generateRefreshToken(c.Request.Context(), user.ID.String())
        response.OK(c, gin.H{
