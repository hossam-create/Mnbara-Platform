package listings_test
  ...
  func TestSearchRequest_Validate(t *testing.T) {
  	tests := []struct {
  		...
  		{
  			name:    "negative price",
  			req:     listings.SearchRequest{MinPrice: -10},
  			wantErr: true,
  		},
  		{
  			name:    "min > max",
  			req:     listings.SearchRequest{MinPrice: 500, MaxPrice: 100},
  			wantErr: true,
  		},
  		{
  			name:    "invalid geo: lat without lng",
  			req:     listings.SearchRequest{Lat: 25.2, Lng: 0, Radius: 10},
  			wantErr: true,
  		},
  		{
  			name: "valid geo search",
  			req:  listings.SearchRequest{Lat: 25.2, Lng: 55.3, Radius: 50},
  		},
  	}
