package listings
  ...
  func (h *Handler) Search(c *gin.Context) {
        ...
        if req.PerPage < 1 { req.PerPage = 20 }
        if req.PerPage > 100 { req.PerPage = 100 }
        if req.Radius < 1  { req.Radius = 50 }
        if req.SortBy == "" { req.SortBy = "date" }
        ...
        case "relevance":
                if req.Query != "" {
                        safeQuery := strings.ReplaceAll(req.Query, "'", "''")
                        q = q.Order(fmt.Sprintf("ts_rank(to_tsvector('english', listings.title || ' ' || COALESCE(listings.description, '')), plainto_tsquery('english', '%s')) DESC", safeQuery))
                } else {
