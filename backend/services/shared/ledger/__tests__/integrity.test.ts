...
describe('Ledger Integrity Verification', () => {
  ...
  describe('Status Values', () => {
    test('has all status values', () => {
      expect([LedgerEntryStatus.PENDING, LedgerEntryStatus.CONFIRMED, LedgerEntryStatus.FAILED, LedgerEntryStatus.REVERSED]).toHaveLength(4);
    });
  });
  describe('Reversal Entries', () => {
    test('creates reversal with negated amount', () => {
      ...
      const reversal = createMockEntry(2, LedgerEntryType.REFUND, original.entryHash, { amount: -500 });
