...
describe('SearchService Integration Tests', () => {
  ...
  class MockSearchService {
      ...
      const response = await this.client.search({
        ...
        body: {
          ...
          aggs: {
            ...
            price_ranges: { range: { field: 'price', ranges: [{ to: 50 }, { from: 50, to: 100 }, { from: 100, to: 500 }, { from: 500, to: 1000 }, { from: 1000 }] } },
          },
        },
      });
    ...
    async searchAuctions(options: { query: string; sortBy?: string; endingSoon?: boolean }) {
      const { query, sortBy = 'ending_soon', endingSoon } = options;
      const must: unknown[] = [];
      const filter: unknown[] = [];

      if (query && query.trim()) {
        must.push({ multi_match: { query: query.trim(), fields: ['title^3', 'description'], fuzziness: 'AUTO' } });
      } else {
        must.push({ match_all: {} });
      }
      ...
      if (endingSoon) {
        filter.push({ range: { endAt: { lte: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(), gte: new Date().toISOString() } } });
      }

      const sort = sortBy === 'ending_soon' ? [{ endAt: 'asc' }, { _score: 'desc' }] : [{ _score: 'desc' }];

      const response = await this.client.search({
        index: 'mnbara_auctions',
        body: { query: { bool: { must, filter } }, sort },
      });

      const hits = response.hits.hits;
      const total = typeof response.hits.total === 'number' ? response.hits.total : response.hits.total?.value || 0;

      return { items: hits.map((hit: { _source: unknown; _score: number }) => ({ ...(hit._source as object), _score: hit._score })), total, aggregations: response.aggregations };
    }
    ...
    async autocomplete(query: string, limit: number = 10) {
      ...
      const productResponse = await this.client.search({
        index: 'mnbara_products',
        body: { size: limit, query: { bool: { should: [{ match: { 'title.autocomplete': { query: query.trim(), operator: 'and' } } }], filter: [{ term: { status: 'active' } }] } }, _source: ['id', 'title'] },
      });

      for (const hit of productResponse.hits.hits) {
        const source = hit._source as { id: string; title: string };
        results.push({ text: source.title, type: 'product', id: source.id, score: hit._score || 0 });
      }

      const categoryResponse = await this.client.search({
        index: 'mnbara_categories',
        body: { size: 5, query: { match: { 'name.autocomplete': { query: query.trim(), operator: 'and' } } }, _source: ['id', 'name'] },
      });

      for (const hit of categoryResponse.hits.hits) {
        const source = hit._source as { id: string; name: string };
        results.push({ text: source.name, type: 'category', id: source.id, score: hit._score || 0 });
      }
  ...
  describe('Search Relevance Scoring', () => {
    it('should rank title matches higher than description matches', async () => {
      mockSearch.mockResolvedValueOnce({
        hits: { total: { value: 2 }, hits: [{ _id: '1', _score: 15.5, _source: { id: '1', title: 'iPhone 15 Pro Max' } }, { _id: '2', _score: 8.2, _source: { id: '2', title: 'Phone Case' } }] },
        aggregations: {},
      });
      const result = await searchService.searchProducts({ query: 'iPhone' });
    ...
    it('should rank exact matches higher than fuzzy matches', async () => {
      mockSearch.mockResolvedValueOnce({
        hits: { total: { value: 2 }, hits: [{ _id: '1', _score: 20.0, _source: { id: '1', title: 'Samsung Galaxy S24' } }, { _id: '2', _score: 12.5, _source: { id: '2', title: 'Samsng Galaxy S23' } }] },
        aggregations: {},
      });
      const result = await searchService.searchProducts({ query: 'Samsung' });
    ...
    it('should build correct multi_match query with fuzzy matching', async () => {
      ...
      await searchService.searchProducts({ query: 'smartphone' });
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({
        index: 'mnbara_products',
        body: expect.objectContaining({
          query: expect.objectContaining({
            bool: expect.objectContaining({
              must: expect.arrayContaining([expect.objectContaining({ multi_match: expect.objectContaining({ query: 'smartphone', fuzziness: 'AUTO' }) })]),
            }),
          }),
        }),
      }));
    ...
    it('should use match_all query when no search query provided', async () => {
      ...
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({
        body: expect.objectContaining({ query: expect.objectContaining({ bool: expect.objectContaining({ must: expect.arrayContaining([{ match_all: {} }]) }) }) }),
      }));
    });
  });
  ...
  describe('Autocomplete Performance', () => {
    it('should return autocomplete results within performance threshold', async () => {
      mockSearch.mockResolvedValueOnce({ hits: { hits: [{ _score: 10, _source: { id: '1', title: 'iPhone 15 Pro' } }] } }).mockResolvedValueOnce({ hits: { hits: [{ _score: 5, _source: { id: 'cat1', name: 'iPhones' } }] } });
    ...
    it('should return autocomplete results sorted by relevance score', async () => {
      mockSearch.mockResolvedValueOnce({ hits: { hits: [{ _score: 15, _source: { id: '1', title: 'MacBook Pro' } }, { _score: 12, _source: { id: '2', title: 'MacBook Air' } }] } }).mockResolvedValueOnce({ hits: { hits: [{ _score: 8, _source: { id: 'cat1', name: 'MacBooks' } }] } });
    ...
    it('should include both product and category suggestions', async () => {
      mockSearch.mockResolvedValueOnce({ hits: { hits: [{ _score: 10, _source: { id: '1', title: 'Nike Air Max' } }] } }).mockResolvedValueOnce({ hits: { hits: [{ _score: 8, _source: { id: 'cat1', name: 'Nike Shoes' } }] } });
      const results = await searchService.autocomplete('Nike', 10);
      expect(results.filter((r) => r.type === 'product').length).toBeGreaterThan(0);
      expect(results.filter((r) => r.type === 'category').length).toBeGreaterThan(0);
    });

    it('should respect the limit parameter', async () => {
      mockSearch.mockResolvedValueOnce({ hits: { hits: [{ _score: 10, _source: { id: '1', title: 'P1' } }, { _score: 9, _source: { id: '2', title: 'P2' } }, { _score: 8, _source: { id: '3', title: 'P3' } }, { _score: 7, _source: { id: '4', title: 'P4' } }, { _score: 6, _source: { id: '5', title: 'P5' } }] } }).mockResolvedValueOnce({ hits: { hits: [{ _score: 5, _source: { id: 'c1', name: 'C1' } }] } });
  ...
  describe('Search Filters', () => {
    it('should apply price range filters correctly', async () => {
      ...
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({ body: expect.objectContaining({ query: expect.objectContaining({ bool: expect.objectContaining({ filter: expect.arrayContaining([{ range: { price: { gte: 500, lte: 1500 } } }]) }) }) }) }));
    });
    ...
    it('should apply category filter correctly', async () => {
      ...
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({ body: expect.objectContaining({ query: expect.objectContaining({ bool: expect.objectContaining({ filter: expect.arrayContaining([{ term: { categoryId: 'electronics-phones' } }]) }) }) }) }));
    });
    ...
    it('should apply geo-location filter correctly', async () => {
      ...
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({ body: expect.objectContaining({ query: expect.objectContaining({ bool: expect.objectContaining({ filter: expect.arrayContaining([{ geo_distance: { distance: '50km', location: { lat: 30.0444, lon: 31.2357 } } }]) }) }) }) }));
    });
  });
  ...
  describe('Auction Search', () => {
    it('should filter auctions ending soon', async () => {
      ...
      expect(mockSearch).toHaveBeenCalledWith(expect.objectContaining({ index: 'mnbara_auctions', body: expect.objectContaining({ query: expect.objectContaining({ bool: expect.objectContaining({ filter: expect.arrayContaining([{ term: { status: 'active' } }]) }) }) }) }));
    });
  ...
  describe('Pagination', () => {
    ...
    it('should return correct pagination metadata', async () => {
      mockSearch.mockResolvedValueOnce({ hits: { total: { value: 95 }, hits: Array(20).fill({ _id: '1', _score: 5, _source: { id: '1', title: 'Product' } }) }, aggregations: {} });
  ...
  describe('Aggregations', () => {
    it('should return aggregations for faceted filtering', async () => {
      mockSearch.mockResolvedValueOnce({
        ...
        aggregations: { categories: { buckets: [{ key: 'electronics', doc_count: 30 }] }, conditions: { buckets: [{ key: 'new', doc_count: 35 }] }, price_ranges: { buckets: [{ key: '*-50.0', doc_count: 10 }] } },
      });
